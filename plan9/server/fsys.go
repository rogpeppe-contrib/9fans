...
type FsysInner[Fid any, C any] interface {
	Fsys[Fid]
...
type Fsys[Fid any] interface {
	...
	//
	// This method must be safe to call concurrently.
	Auth(ctx context.Context, dst *Fid, uname, aname string) error
	...
	//
	// This method must be safe to call concurrently.
	Attach(ctx context.Context, dst *Fid, auth *Fid, uname, aname string) error

	// Stat returns information about the file, which may be open or not.
	//
	// This method must be safe to call concurrently.
	Stat(ctx context.Context, f *Fid) (plan9.Dir, error)

	// Wstat writes metadata about the file.
	//
	// This method must be safe to call concurrently.
	// TODO should we make this exclusive?
	Wstat(ctx context.Context, f *Fid, dir plan9.Dir) error

	// Walk walks f to the named element within the directory
	// represented by f .
	//
	// This method will never be called concurrently on the same f.
	Walk(ctx context.Context, f *Fid, name string) error
	...
	//
	// This method will never be called concurrently on the same f.
	Open(ctx context.Context, f *Fid, mode uint8) (uint32, error)
	...
	//
	// This method will never be called concurrently on the same f.
	Readdir(ctx context.Context, f *Fid, dir []plan9.Dir, entryIndex int) (int, error)

	// ReadAt reads data from f into buf at the file offset off.
	//
	// This method must be safe to call concurrently.
	ReadAt(ctx context.Context, f *Fid, buf []byte, off int64) (int, error)

	// WriteAt writes data from buf into f at the file offset off.
	//
	// This method must be safe to call concurrently.
	WriteAt(ctx context.Context, f *Fid, buf []byte, off int64) (int, error)
